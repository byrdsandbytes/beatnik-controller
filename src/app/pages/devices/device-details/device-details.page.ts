...
export class DeviceDetailsPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.serverState.subscribe((state) => {
      ...
      if (group) {
        ...
        console.error(
          `DeviceDetailsPage: Group with ID ${this.id} not found in server state`
        );
      }
    });
  ...
  changeGroupName(newName: string): void {
    ...
    this.snapcastService.setGroupName(this.id, newName).subscribe({
      next: () => {
        console.log(
          `DeviceDetailsPage: Group name changed to ${newName} for ID ${this.id}`
        );
      },
      error: (error) => {
        console.error(
          `DeviceDetailsPage: Error changing group name for ID ${this.id}`,
          error
        );
      },
    });
  ...
  changeStream(streamId: string): void {
    ...
    this.snapcastService.setGroupStream(this.id, streamId).subscribe({
      next: () => {
        console.log(
          `DeviceDetailsPage: Stream changed to ${streamId} for ID ${this.id}`
        );
      },
      error: (error) => {
        console.error(
          `DeviceDetailsPage: Error changing stream for ID ${this.id}`,
          error
        );
      },
    });
  ...
  preselectClientFirstClient(id: string): void {
    this.serverState.subscribe((state) => {
      ...
      if (group && group.clients && group.clients.length > 0) {
        ...
        console.log(
          'DeviceDetailsPage: Preselected first client:',
          this.selectedClient
        );
      } else {
