...
export class SetupSoundcardPage implements OnInit {
  ...
  async openSoundcardPicker() {
    ...
    if (data && data.selectedHatId) {
      if (hardwareStatus.currentConfig.id === data.selectedHatId) {
        console.log(
          'SetupSoundcardPage: Selected hat is the same as current configuration, no changes needed'
        );
        return;
      } else {
        console.log(
          'SetupSoundcardPage: Soundcard selected:',
          data.selectedHatId
        );
      ...
      console.log(
        'SetupSoundcardPage: Soundcard selection cancelled or no selection made'
      );
    }
  }
      ...
      buttons: [
        {
          ...
          handler: () => {
            console.log(
              'SetupSoundcardPage: Soundcard change cancelled by user'
            );
          },
        },
        {
          ...
          handler: () => {
            console.log(
              'SetupSoundcardPage: User chose to apply soundcard changes'
            );
  ...
  async applySoundcardConfig(hatId: string) {
    console.log(
      `SetupSoundcardPage: Applying hardware configuration ${hatId} to client ${this.ip}`
    );
    ...
    this.beatnikHardwareService
      ...
      .subscribe({
        next: (response) => {
          console.log(
            `SetupSoundcardPage: Successfully applied hardware configuration ${hatId} to client ${this.ip}`,
            response
          );
          this.showRebootInfo();
          if (response.rebootRequired) {
            console.log(
              'SetupSoundcardPage: Reboot required. Triggering reboot...'
            );
            this.beatnikHardwareService.reboot(localHostName).subscribe({
              next: () => {
                console.log(
                  `SetupSoundcardPage: Successfully triggered reboot for client ${this.ip}`
                );
              },
              error: (err) => {
                console.error(
                  `SetupSoundcardPage: Failed to trigger reboot for client ${this.ip}`,
                  err
                );
              },
            });
        ...
        error: (err) => {
          console.error(
            `SetupSoundcardPage: Failed to apply hardware configuration ${hatId} to client ${this.ip}`,
            err
          );
        },
      });
      ...
      buttons: [
        {
          ...
          handler: () => {
            console.log(
              'ClientDetailsPage: User acknowledged reboot requirement'
            );
  ...
  waitForReboot() {
    console.log(
      'SetupSoundcardPage: Waiting for client to reboot and come back online...'
    );
    ...
    const intervalId = setInterval(() => {
      this.beatnikHardwareService.getStatus(this.ip).subscribe({
        next: (status) => {
          console.log(
            'SetupSoundcardPage: Client is back online after reboot',
            status
          );
          clearInterval(intervalId);
          this.hardwareStatus$ = new Observable<HardwareStatus>((observer) =>
        ...
        error: (err) => {
          console.log(
            'SetupSoundcardPage: Client is still offline, waiting...',
            err
          );
          if (Date.now() - startTime > timeOut) {
            console.error(
              'SetupSoundcardPage: Timeout waiting for client to reboot'
            );
            clearInterval(intervalId);
            this.state = 'error';
          }
  ...
  completeSetup() {
    console.log(
      'SetupSoundcardPage: User completed soundcard setup, navigating back to client details'
    );
