...
export class DashboardPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.displayState.subscribe((state) => {
      if (state) {
        ...
        console.warn(
          'SnapcastStateIndicatorService: No state available to check for playing streams and clients.'
        );
      }
    });
    ...
    interval(1000).subscribe(() => {
      ...
      this.lastServerResponseDeltaInSeconds = Math.floor(
        (today.getTime() -
          (this.lastServerResponseTime?.getTime() || today.getTime())) /
          1000
      );
    });
  }
  ...
  async checkForPlayingStreamsAndClients(): Promise<void> {
    console.log(
      'PlayerToolbarComponent: Checking for playing streams and clients...'
    );
    const state = await firstValueFrom(this.displayState);
    if (!state) {
      console.warn(
        'PlayerToolbarComponent: No state available to check for playing streams and clients.'
      );
      return;
    }
    // Check if there are any playing streams
    this.hasPlayingStreams = state.server.streams.some(
      (stream) => stream.status === 'playing'
    );
    console.log(
      'PlayerToolbarComponent: hasPlayingStreams:',
      this.hasPlayingStreams
    );
    ...
    this.hasAnyPlayingClients = state.server.groups.some((group: Group) =>
      group.clients.some((client: Client) => {
        const clientStream = state.server.streams.find(
          (stream) => stream.id === group.stream_id
        );
        return (
          client.connected && clientStream && clientStream.status === 'playing'
        );
      })
    );
    console.log(
      'PlayerToolbarComponent: hasAnyPlayingClients:',
      this.hasAnyPlayingClients
    );
    this.numberOfPlayingClients = state.server.groups.reduce((count, group) => {
      return (
        ...
        group.clients.filter(
          (client) =>
            client.connected &&
            state.server.streams.some(
              (stream) =>
                stream.id === group.stream_id && stream.status === 'playing'
            )
        ).length
      );
    }, 0);
    console.log(
      'PlayerToolbarComponent: numberOfPlayingClients:',
      this.numberOfPlayingClients
    );
    this.numberOfPlayingStreams = state.server.streams.filter(
      (stream) => stream.status === 'playing'
    ).length;
    console.log(
      'PlayerToolbarComponent: numberOfPlayingStreams:',
      this.numberOfPlayingStreams
    );

    this.totalClients = state.server.groups.reduce(
  ...
  loadSpeakerData(): void {
    this.http
      .get<{ speakers: Speaker[] }>('assets/speakers/speakers-data.json')
      .subscribe({
        next: (response) => {
  ...
  refreshSeverState(event: RefresherCustomEvent): void {
    ...
    this.snapcastService.refreshState().subscribe({
      next: () => {
        ...
        setTimeout(() => {
          this.refreshingText =
            'Server state refreshed successfully at ' +
