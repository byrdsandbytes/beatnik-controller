...
} from 'src/app/model/snapcast.model'; // Adjust path
...
export class SnapcastStatusComponent implements OnInit, OnDestroy {
  ...
  getClientStats(state: SnapCastServerStatusResponse): {
    total: number;
    online: number;
    offline: number;
  } {
  ...
  onSetClientVolumePercent(clientId: string, event: Event): void {
    ...
    this.subscriptions.add(
      this.snapcastService
        .setClientVolumePercent(clientId, volume)
        .pipe(
          catchError((err) => {
            console.error(
              `Component: Failed to set volume for ${clientId}`,
              err
            );
            return EMPTY;
          })
