...
import {
  ...
  MixerSource,
  SignalLevels,
} from 'src/app/model/camilla-dsp.model';
import {
  CamillaDspService,
  ConnectionStatus,
} from 'src/app/services/camilla-dsp.service';
...
export class CamillaDspComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.levelSubscription = this.camillaService.signalLevels$.subscribe(
      (levels) => {
  ...
  updateFilterParameter(filterKey: string, paramKey: string, newValue: any) {
    ...
    this.camillaService.sendCommand(
      'SetConfigJson',
      JSON.stringify(this.parsedConfig)
    );
  }
  ...
  getVolume() {
    ...
    this.currentVolume =
      this.lastMessage.GetVolume?.value ?? this.currentVolume;
  ...
  ionViewWillLeave() {
    console.log(
      'CamillaDspComponent: Leaving page, cleaning up resources if needed'
    );
    this.ngOnDestroy();
  }
}
